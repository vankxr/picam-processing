...
void process_frame(int n, uint8_t* curr, uint8_t* prev, uint8_t* filter)
{
    for(int i = 0; i < n; i++)
    {
        uint8_t icurr = curr[i];
        uint8_t iprev = prev[i];
        uint8_t ifilter = filter[i];
        ...
        int filtered = ((int)ifilter * ((1 << IIR_COEF_LOG2) - 1) + (int)diff) >> IIR_COEF_LOG2;

        curr[i] = filtered;
        filter[i] = filtered;
    }
}
void process_frame_no_filter(int n, uint8_t* curr, uint8_t* prev, uint8_t* filter)
{
    for(int i = 0; i < n; i++)
    {
        uint8_t icurr = curr[i];
...
int main(int argc, char **argv)
{
    ...
    uint8_t                         current[FRAME_WIDTH * FRAME_HEIGHT];
    uint8_t                         prev[FRAME_WIDTH * FRAME_HEIGHT];
    uint8_t                         filter[FRAME_WIDTH * FRAME_HEIGHT];
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        if(!i)
            process_frame_no_filter(n, current, prev, filter);
        else
            process_frame(n, current, prev, filter);
